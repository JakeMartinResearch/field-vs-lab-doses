...
```{r}
concentration_str <- bf(value_log ~ surfacewater_median_log,
                  family = gaussian())

concentration_str_null <- bf(value_log ~ 1,
                  family = gaussian())
```
...
Pulling the conditional effect estimates from the model. The estimate corresponds to percent change in predictor `log(value)` for a percent change in the predictor `log(surfacewater_median)`. Effect of surface water median (log) is 0.1167 (β1)), which implies that a 1% increase in surface water median concentration is associated with a 0.117% increase in tested concentration (since log-log models estimate elasticities).The 95% credible interval (-0.1403, 0.3770) overlap with zero, suggesting this not a statistically meaningful effects.  


```{r}
# Extract estimates
esitmates_3 <- conditional_effects(concentration_mod_log)
estimate_data_3 <- esitmates_3$surfacewater_median
 

# Extract the summary as a data frame
model_summary_3 <- as.data.frame(summary(concentration_mod_log, prob = 0.95)$fixed) %>% 
  rownames_to_column() %>% 
  clean_names()

estimate_3 <- model_summary_3$estimate[2]
L95CI_3 <- model_summary_3$l_95_percent_ci[2]
H95CI_3 <- model_summary_3$u_95_percent_ci[2]
...
fig_1a <- ggplot() +
  geom_line(data = estimate_data_3, aes(x = surfacewater_median_log, y = estimate__), color = "#1c4595", linewidth = 1) +
  geom_ribbon(data = estimate_data_3, aes(x = surfacewater_median_log, ymin = lower__, ymax = upper__), fill = "#1c4595", alpha = 0.1) +
  ...
  annotate(
    ...
    label = paste0(
      ...
      "Effect estimate (β): ", round(estimate_3, 3), "\n",
      "CI: ", round(L95CI_3, 3), " - ", round(H95CI_3, 3)
    ),
...
```{r}
eff_concentration_str <- bf(value_log ~ effluent_median_log,
                  family = gaussian())

eff_concentration_str_null <- bf(value_log ~ 1,
                  family = gaussian())
```
...
```{r}
# Extract estimates
esitmates_4 <- conditional_effects(eff_concentration_mod_log)
estimate_data_4 <- esitmates_4$effluent_median_log
 

# Extract the summary as a data frame
model_summary_4 <- as.data.frame(summary(eff_concentration_mod_log, prob = 0.95)$fixed) %>% 
  rownames_to_column() %>% 
  clean_names()

estimate_4 <- model_summary_4$estimate[2]
L95CI_4 <- model_summary_4$l_95_percent_ci[2]
H95CI_4 <- model_summary_4$u_95_percent_ci[2]
...
fig_1b <- ggplot() +
  geom_line(data = estimate_data_4, aes(x = effluent_median_log, y = estimate__), color = "#1c4595", linewidth = 1) +
  geom_ribbon(data = estimate_data_4, aes(x = effluent_median_log, ymin = lower__, ymax = upper__), fill = "#1c4595", alpha = 0.1) +
  ...
  annotate(
    ...
    label = paste0(
      "Bayesian log-log Gaussian regression", "\n",
      "Effect estimate: ", round(estimate_4, 3), "\n",
      "CI: ", round(L95CI_4, 3), " - ", round(H95CI_4, 3)
    ),
    hjust = 0, vjust = 1, size = 4
  ) +
  labs(
...
invisible(fig_S5 <- plot_left | plot_right)
```

**Fig S5**: The median fold difference in tested concentrations in EIPAAB and median waste water concentration (i.e. tested dose / median waste water concentration) 
...
This is also fig 1a in the main manuscript. 

**Fig 3a:** the relationship between the number of surface water samples for pharmaceuticals in environmental databases (Norman, UBA and Wilkson) and the occurrence in the Evidence of the Impacts of Pharmaceuticals on Aquatic Animal Behaviour (EIPAAB) database.
...
**Fig 3b:** the relationship between the percentage of surface water detections for pharmaceuticals in environmental databases (Norman, UBA and Wilkson) and the occurrence in the Evidence of the Impacts of Pharmaceuticals on Aquatic Animal Behaviour (EIPAAB) database.   
